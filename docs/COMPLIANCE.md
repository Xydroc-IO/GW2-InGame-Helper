...
- Public item/currency **name + icon catalog**, **station recipes**, and **unique official render PNGs** from GitHub Releases tag `gw2-helper-catalog` (`gw2-helper-catalog.manifest` + `gw2-helper-catalog.igh` / `gw2-helper-icons.igh`; IGH1; no API keys; cache under `addons/.../cache/`). ArenaNet retains ownership of those textures; they are **not** relicensed under MIT. The **CEF runtime zip** is on that same tag.
