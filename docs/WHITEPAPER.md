...
|------------|-------|-----------------------------|
| 2.3.0.7 | 2.3.0.7 | World-map path overlay (TacO scale); Paths QA; heart tips with MC; Lua hot toggle; compass no avatar align; stamps unchanged |
| 2.3.0.6 | 2.3.0.6 | Lady With Mounts Caledon world GPS gaps; compass overlay align + TacO pivot (no orbit on turn); stamps unchanged |
...
| 2.2.4.8 | 2.2.4.8 | JW/VoE Ledger catalog; blank Ledger JS fix; live 71 |
...
| 2.2.3.0 | 2.2.3.0 | Immersive ImGui chrome (Blish StandardWindow + custom title bar); ui-chrome pack uc6; pad clip/on-screen polish |
...
7. Documentation of the **application layer** at architecture impact level: Pathing + D3D world GPS, Account API pads, DPS Logs / Elite Insights, companions (Economy / Instances), and display overlays.
...
7. **New addon tabs** to `file://` / `about:` only when the initiating frame is a bundled helper page. `https` may not open local files (chrome Home hops `about:blank` first so the hub `file://` can commit).
...
1. CEF 150+ Windows sandbox expects `windows_sandbox_info` from official **MSVC** `cef_sandbox.lib`. This helper is MinGW C API — we cannot pass that pointer, and `no_sandbox=0` without it fails `cef_initialize`.
2. Wine/Proton sandboxing is unreliable.
3. Many embedded CEF apps make the same trade.

This is a **toolchain limit**, not an accidental leftover flag. A native-Windows sandbox would be a second (MSVC) helper binary.
...
6. **Documentation lag.** This report tracks revision 2.2.3.10; future commits may land before the next sync.
...
As of revision 2.2.3.10 the Browse catalog is data-driven (`data/sites.json`), and former monolithic translation units are split into focused units (prefer ≤500 lines per `.cpp`). This reduces merge-conflict surface for feature work but does **not** remove restricted ownership of the CEF, IPC, and present path. See [`MODULES.md`](MODULES.md), [`ARCHITECTURE.md`](ARCHITECTURE.md) §7, [`KERNEL.md`](KERNEL.md).
...
**Economy** and **Instances** are read-only side-rail companions (commerce Flip Finder / charts / cart; story/fractal/raid/strike journal). **Stash** and **Crafting** are their own rails (wallet/bank/bags/materials; plan / known / browse / cart). Crafting station trees load from `gw2-helper-recipes.igh`; known recipes still use the account API. **Account → Progress** groups the legendary armory with the embedded Ledger catalog. **Unlocks** groups dyes by hue, skins by armor/weapon/slot, and other kinds into lettered icon tiles (cloth RGB + wardrobe icon URLs, not item-atlas ids). **Achievements** loads groups/defs from `gw2-helper-achievements.igh` and overlays `/v2/account/achievements`. Persist under `config/` (`achievement-pins.txt`).
...
This report is a **design reconstruction** from the shipping codebase and maintainer operational knowledge at revision 2.2.3.10. It is not the output of a formal measurement campaign. Where quantitative figures appear (frame bytes, ring sizes, bandwidth upper bounds), they are derived from constants in [`WikiIpc.h`](../src/browser/WikiIpc.h) and elementary arithmetic unless otherwise stated.
...
## Appendix A — Quantitative constants (revision 2.3.0.7)

| Constant | Value |
|----------|-------|
| Addon version | 2.3.0.7 |
| Nexus signature | `HELP` / `0x48454C50` |
...
| `src/helper/main.cpp` + State/Paths/Resolve/BrowseActions/Tabs/Handlers/Commands | CEF boot, tabs, IPC drain |
...
3. Pathing search guide sets the orange trail; GPS arrow points at the destination.
...
| Last sync | 2.3.0.7 — World-map overlay + Paths QA; heart tips; Lua hot toggle; helper 2247; live 86; home 2238; sites s2215; c2228; raid food 9; watchd w10; uc36 |
