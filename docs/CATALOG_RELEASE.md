...
| `gw2-helper-recipes.igh` | **IGH1** raw members: `recipes.ver`, `recipes.tsv` (~700KB). Crafting loads this before the full names pack finishes downloading. Same TSV as inside `gw2-helper-catalog.igh`. |
...
- https://github.com/Xydroc-IO/GW2-InGame-Helper/releases/download/gw2-helper-catalog/gw2-helper-recipes.igh
...
The addon GETs the GitHub manifest **once per shipping DLL version** (and while any pack is still missing). It does **not** poll every 6 hours. The local cache file may add an `addon` field (`2.3.0.x`) after a successful GET — do **not** upload that key to GitHub. A **cold** cache downloads **all** missing packs in that same worker: `catalog.igh`, `recipes.igh`, `achievements.igh`, and `icons.igh`. Returning players whose cache already matches the manifest `catalog` / `icons` ids skip the big files. Daily catalog rebuilds on the GitHub tag therefore wait until the next shipping DLL (or a wiped cache). The CEF zip is once per machine (`addons/.../cef/`), not every helper update. A matching icons hash must not skip the names pack. `gw2-helper-recipes.igh` (~700KB) still downloads on its own when crafting needs station recipes before the full names pack lands. `gw2-helper-achievements.igh` downloads when missing or when `ach.ver` does not match the manifest `catalog` id. Names/recipes/achievement TSVs inside the igh files are **raw** (not gzip members) so Wine does not have to unwrap a gzip-in-gzip body. If GitHub still Content-Encoding-gzip-wraps the whole file, the DLL unwraps that outer layer before reading IGH1. Any missing pack is retried every 30 minutes (not a tight loop). The shipping DLL is never downloaded from this tag.

The **GW2 Helper Catalog** Action only clobbers **catalog manifest + names igh + recipes igh + achievements igh**. It must **not** delete or replace the icons pack or the CEF zip. Daily catalog rebuilds merge into the existing manifest so the `icons` / `cef` fields stay. Never `gh release delete gw2-helper-catalog`.

Offline: drop `gw2-helper-catalog.igh`, `gw2-helper-recipes.igh`, `gw2-helper-achievements.igh`, and/or `gw2-helper-icons.igh` next to the DLL or in `addons/GW2-InGame-Helper/`.

## What this is / is not

- **Is:** public names, a split station-recipes pack, achievement groups+defs, unique render icons rebuilt when the API build (or icon set) changes, plus the CEF runtime zip. ArenaNet retains ownership of those textures; they are not MIT-relicensed.
...
gh release upload gw2-helper-catalog dist/gw2-helper-catalog.manifest \
  dist/gw2-helper-catalog.igh dist/gw2-helper-recipes.igh dist/gw2-helper-achievements.igh --clobber
```

Recipes pack only (~2 min; skip the item/name crawl):

```bash
python3 scripts/build-gw2-catalog.py -o dist --recipes-only
gh release upload gw2-helper-catalog dist/gw2-helper-recipes.igh --clobber
```

Achievements pack only (~3–5 min; skip the item/recipe crawl):

```bash
python3 scripts/build-gw2-catalog.py -o dist --achievements-only
gh release upload gw2-helper-catalog dist/gw2-helper-achievements.igh --clobber
```
