...
namespace EconomyDetail
{
	...
	static int gFlipView = 1; /* 0 All | 1 Top 10 — default Top 10 */
	...
	static void DrawFlipRow(const FlipRow& r, int rank)
	{
		ImGui::PushID(r.id);
		if (rank > 0)
		{
	...
	void DrawFlipsTab()
	{
		...
		if (ImGui::RadioButton("Top 10 Flips###gw2igh_fv1", gFlipView == 1))
			gFlipView = 1;
		ImGui::SameLine();
		if (ImGui::RadioButton("All###gw2igh_fv0", gFlipView == 0))
			gFlipView = 0;

		const bool top10 = (gFlipView == 1);
		if (!top10)
		{
		...
		PadLayout::BeginList(top10 ? "###gw2igh_eco_flips_top" : "###gw2igh_eco_flips");
		int shown = 0;
		int rank = 0;
		...
		if (gFlips.empty() && !gFlipBusy)
			...
			ImGui::TextColored(HelperTheme::Muted,
				top10 ? "No positive-net flips in this scan." : "No items match this filter.");
		PadLayout::EndList();
	}
}
